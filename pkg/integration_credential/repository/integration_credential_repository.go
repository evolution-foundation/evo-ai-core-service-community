...
type IntegrationCredentialRepository interface {
	...
	Update(ctx context.Context, credential *model.IntegrationCredential, id uuid.UUID) (*model.IntegrationCredential, error)
...
func (r *integrationCredentialRepository) Update(ctx context.Context, credential *model.IntegrationCredential, id uuid.UUID) (*model.IntegrationCredential, error) {
	credential.UpdatedAt = time.Now()
	if err := r.db.WithContext(ctx).Where("id = ?", id).Updates(credential).First(&credential).Error; err != nil {
		return nil, err
	}
